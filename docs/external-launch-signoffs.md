...
- [ ] Confirm Redis is configured for normal production. In staging with two app instances, prove both increment the same shared limit; then disconnect Redis and prove readiness and protected traffic fail closed rather than silently switching to process memory.
