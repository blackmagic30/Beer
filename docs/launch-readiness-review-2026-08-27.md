# PintPath launch-readiness review — 27 August 2026
...
| Permanent staging application | The public Railway route currently returns `Application not found` for `/health`, `/startup`, and `/ready`. Underlying PostgreSQL, Redis, Supabase/Auth, and private Storage identities have historical synthetic evidence, but no current candidate application is serving or proved there. |
...
worktree also contains a broader pre-existing iOS redesign. The combined diff is
not committed, merged, independently approved, deployed, or a release
candidate. Isolate or explicitly review that broader iOS work before creating a
focused PR; freeze only the resulting reviewed SHA after permanent staging
passes.
...
- Repaired bounded iOS launch-safety defects in legal-consent sequencing,
  account recovery/accessibility, contribution navigation, and serving-aware
  fallback presentation. iOS now also emits bounded consent-gated search
  usefulness and supports the signed one-tap confirmation outcomes with
  adaptive 44-point controls. The broader pre-existing iOS redesign still
  requires review as part of candidate isolation.
...
- The complete pinned-Node-22 `npm run check` passed: 233 test files and 4,265
  ...
  environment gates. The security scan checked 925 tracked/untracked files and
