...
function ownDataJson(value: unknown, depth = 0): string | null {
  ...
  if (typeof value === "string" || typeof value === "boolean") {
    return REFLECT_APPLY_EXACT(JSON_STRINGIFY_EXACT, JSON, [value]) as string;
  }
  if (typeof value === "number") {
    return NUMBER_IS_FINITE_EXACT(value)
      ? REFLECT_APPLY_EXACT(JSON_STRINGIFY_EXACT, JSON, [value]) as string
      : null;
  }
  if (typeof value !== "object") return null;
  if (ARRAY_IS_ARRAY_EXACT(value)) {
    if (OBJECT_GET_PROTOTYPE_OF_EXACT(value) !== Array.prototype) return null;
    const descriptors = OBJECT_GET_OWN_PROPERTY_DESCRIPTORS_EXACT(value) as
      unknown as Record<PropertyKey, PropertyDescriptor>;
    const length = descriptors.length?.value;
    if (!NUMBER_IS_SAFE_INTEGER_EXACT(length) || length < 0 || length > 2_000) {
      return null;
    }
    let output = "[";
...
function canonicalSha256(domain: string, value: unknown): string {
  ...
  if (canonical === null) throw new Error("authority_invalid");
  ...
  if (
    ...
    || !REFLECT_APPLY_EXACT(BUFFER_IS_BUFFER_EXACT, Buffer, [digest])
    || !REFLECT_APPLY_EXACT(ARRAY_BUFFER_IS_VIEW_EXACT, ArrayBuffer, [digest])
    || OBJECT_GET_PROTOTYPE_OF_EXACT(digest) !== Buffer.prototype
  ...
  if (hex.length !== 64) throw new Error("authority_invalid");
  return hex;
}
