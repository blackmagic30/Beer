...
function patchProjection(value: unknown): PatchProjection | null {
  const createdAt = record(value) ? timestamp(value.createdAt) : null;
  const updatedAt = record(value) ? timestamp(value.updatedAt) : null;
  const appliedAt = record(value) && value.appliedAt !== null
  ...
  if (!exactKeys(value, [
    ...
    typeof value.status !== "string" || createdAt === null ||
    updatedAt === null || createdAt > updatedAt ||
    !(value.appliedAt === null || timestamp(value.appliedAt) !== null) ||
    (value.appliedAt !== null && (appliedAt === null || appliedAt > updatedAt)) ||
...
function suffixPatchExact(
  ...
  return inWindow(row.createdAt, window) && inWindow(row.updatedAt, window) &&
    row.appliedAt !== null && inWindow(row.appliedAt, window) &&
    row.status === "COMMITTED" && row.message === "Setting 2 variables" &&
    Number.isFinite(appliedAt) &&
    appliedAt <= Date.parse(row.updatedAt) &&
    createdAt <= Date.parse(row.updatedAt) &&
