...
function verifyDeployment(value: Json, candidateSha: string): {
  ...
  if (!receipt) fail("production_deployment_invalid");
  return receipt;
}
...
export async function attestProductionPromotionRecovery(
  ...
  const pitr = verifyPitr(json("pitr-receipt"), candidateSha, scale.deploymentIdSha256);
  ...
  const receipt = buildProductionPromotionRecoveryReceipt({
    ...
    productionDeploymentIdSha256: scale.deploymentIdSha256,
