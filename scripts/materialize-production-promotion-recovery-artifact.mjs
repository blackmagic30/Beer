...
const STAGE_CONTRACTS = Object.freeze({
  deploy: Object.freeze({
    phases: Object.freeze(["close", "promotion-recovery"]),
  ...
  close: Object.freeze({
    phases: Object.freeze(["promotion-recovery", "open"]),
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!["--authority", "--candidate-sha", "--stage", "--output"].includes(key)
      || values.has(key) || typeof value !== "string" || value.length === 0) {
  ...
  if (!path.isAbsolute(authority) || !SHA.test(candidateSha)
    || !Object.hasOwn(STAGE_CONTRACTS, stage) || !path.isAbsolute(output)) {
    throw new Error("argument_invalid");
  }
...
function readAuthority(filename, candidateSha, stage) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || stat.nlink !== 1
    || stat.size <= 1 || stat.size > MAX_RECEIPT_BYTES
    || fs.realpathSync(filename) !== filename) throw new Error("authority_invalid");
  ...
  if (canonical(value) !== source || !exact(value, [
    ...
    || !Array.isArray(value.productionChain)) throw new Error("authority_invalid");
  const matches = value.productionChain.filter((item) => item?.stage === stage);
  ...
  if (!exact(artifact, [
    ...
    || !Number.isSafeInteger(artifact.artifactId) || artifact.artifactId <= 0
    ...
    || !Number.isSafeInteger(artifact.runId) || artifact.runId <= 0
...
async function github(fetchImpl, token, endpoint, accept) {
  return fetchImpl(`https://api.github.com/repos/blackmagic30/Beer${endpoint}`, {
    method: "GET",
    headers: {
      Accept: accept,
  ...
  try {
    ...
    if (entries.length < 1 || entries.length > 64
      ...
      || entries.some((entry) => {
        ...
        return candidate.length < 1 || candidate.length > 512
    ...
    const extracted = spawnSync("/usr/bin/unzip", ["-p", archivePath, receiptEntry], {
      encoding: null,
      maxBuffer: MAX_RECEIPT_BYTES,
      shell: false,
    });
    if (extracted.status !== 0 || extracted.signal !== null || extracted.error
      || !Buffer.isBuffer(extracted.stdout)
      || extracted.stdout.length <= 1 || extracted.stdout.length > MAX_RECEIPT_BYTES) {
      throw new Error("artifact_archive_invalid");
    }
    return extracted.stdout;
  } finally {
    try { fs.unlinkSync(archivePath); } catch { /* private temporary file may be absent */ }
  }
}
...
export async function runProductionPromotionRecoveryArtifactMaterializer(
  ...
  try {
    ...
    if (env.GITHUB_ACTIONS !== "true" || env.GITHUB_REPOSITORY !== "blackmagic30/Beer"
      || env.GITHUB_SHA !== args.candidateSha || env.GITHUB_RUN_ATTEMPT !== "1"
      || typeof env.GITHUB_TOKEN !== "string" || env.GITHUB_TOKEN.length < 16
      || /[\r\n\0]/.test(env.GITHUB_TOKEN)) throw new Error("github_context_invalid");
    ...
    if (metadata?.id !== artifact.artifactId || metadata?.name !== artifact.name
      || metadata?.digest !== artifact.digest || metadata?.size_in_bytes !== artifact.sizeBytes
      || metadata?.expired !== false || metadata?.workflow_run?.id !== artifact.runId
    ...
    if (`sha256:${crypto.createHash("sha256").update(archive).digest("hex")}`
      !== artifact.digest) throw new Error("artifact_digest_mismatch");
    const source = extractEntry(archive, path.dirname(args.output), contract.receiptEntry);
    ...
    writeOutput(`${JSON.stringify({
      command: "materialize-production-promotion-recovery-artifact",
      failureCode: error instanceof Error ? error.message.split(":", 1)[0] : "unexpected_failure",
      ok: false,
    })}\n`);
    return 1;
  }
