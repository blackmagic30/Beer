...
export const POST_Q_AUTHORITY_V2_SCHEMA =
  "pintpath-permanent-staging-post-q-authority/v2";
export const POST_Q_REVIEWED_CANDIDATE_V2_SCHEMA =
  "pintpath-permanent-staging-post-q-reviewed-candidate-authority/v2";
...
const SUCCESSOR_EXPLICIT_EXPIRY = "2026-09-09T08:00:00.000Z";
...
const SUCCESSOR_AUTHORIZATION_SOURCE_SIZE_BYTES = 267;
...
export function authorizationSourceExact(source) {
  ...
  return Buffer.byteLength(source) === SUCCESSOR_AUTHORIZATION_SOURCE_SIZE_BYTES &&
    ...
    JSON.stringify(Object.keys(value)) ===
      JSON.stringify(["schemaVersion", "threadId", "messages"]) &&
...
export async function verifyPostQReviewedCandidate(
  ...
  if (!Number.isFinite(currentStartedAt) || !Number.isFinite(mergedAt) ||
    !Number.isFinite(recoveryRunCompletedAt) || !Number.isFinite(expiredDeadline) ||
    !Number.isFinite(nowMs) || currentStartedAt < expiredDeadline ||
    nowMs < expiredDeadline ||
    derivedDeadline === null || !Number.isFinite(deadline) ||
    mergedAt <= recoveryRunCompletedAt || mergedAt > currentStartedAt ||
  ...
  let mainReference;
  try {
    [candidateCommit, predecessorCommit, expiredRunHeadCommit,
  ...
  if (!record(candidateCommit) || candidateCommit.sha !== candidateSha ||
    ...
    predecessorCommit.parents[0]?.sha !== EXPIRED_RUN_HEAD_SHA ||
    ...
    predecessorReviewedHeadCommit.tree.sha !== SUCCESSOR_DIRECT_PARENT_TREE_SHA ||
    ...
    predecessorPullRequest.merge_commit_sha !== SUCCESSOR_DIRECT_PARENT_SHA ||
  ...
  return Object.freeze({
    ...
    predecessorBridge: {
      ...
      soleParentSha: EXPIRED_RUN_HEAD_SHA,
      ...
      mergeCommitSha: SUCCESSOR_DIRECT_PARENT_SHA,
      baseSha: EXPIRED_RUN_HEAD_SHA,
...
export async function verifyPermanentStagingPostQAuthorityV2(
  overrides = {},
) {
  const dependencies = {
  ...
  if (!record(reviewedCandidate) ||
    reviewedCandidate.schemaVersion !== POST_Q_REVIEWED_CANDIDATE_V2_SCHEMA) {
    fail("reviewed_candidate_invalid");
  }
  ...
  const authority = {
    schemaVersion: POST_Q_AUTHORITY_V2_SCHEMA,
    operation: "post-q-deployment-stop-containment-v2",
    repository: REPOSITORY,
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  verifyPermanentStagingPostQAuthorityV2().catch((error) => {
