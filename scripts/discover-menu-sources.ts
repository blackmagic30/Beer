...
async function maybeExtractImageOcr(candidate: MenuSourceCandidate, openai: OpenAI | null): Promise<MenuImageOcrResult | null> {
  ...
  try {
    ...
    const prompt = [
      ...
      "Do not include standalone gin, vodka, whisky, bourbon, tequila, cocktails, wine, food, steak, venue welcome copy, promo copy, category descriptions, or event text as beer rows.",
      "Keep a clearly labelled packaged premixed RTD only when it appears in a beer, cider, RTD, tin, can, bottle, or packaged-drinks section.",
