...
export const POST_Q_AUTHORITY_V3_SCHEMA =
  "pintpath-permanent-staging-post-q-authority/v3";
export const POST_Q_REVIEWED_CANDIDATE_V3_SCHEMA =
...
const SUCCESSOR_AUTHORIZATION_THREAD_ID =
  "01a02140-8628-7d30-9374-8d29d4a9f3a3";
...
const SUCCESSOR_AUTHORIZATION_SOURCE_SIZE_BYTES = 394;
...
export function successorPolicyExact(source) {
  ...
  return source === canonical(value) && sha256(source) === SUCCESSOR_POLICY_SHA256 &&
    ...
    value.workflow.requiredRunNumber === 2 &&
    value.workflow.requiredTotalHistoryRows === 2 &&
    ...
    value.archivedV2.candidateSha === SUCCESSOR_DIRECT_PARENT_SHA &&
    value.archivedV2.candidateTreeSha === SUCCESSOR_DIRECT_PARENT_TREE_SHA &&
...
function ineligibleV2PullRequestExact(value) {
  return record(value) && value.number === INELIGIBLE_V2_RUN.pullRequestNumber &&
    ...
    value.merge_commit_sha === SUCCESSOR_DIRECT_PARENT_SHA &&
    ...
    value.base.sha === SUCCESSOR_DIRECT_PARENT_PARENT_SHA &&
...
function ineligibleV2JobsExact(value) {
  ...
  for (const expected of INELIGIBLE_V2_JOBS) {
    ...
    if (ids.has(job.id) || job.name !== expected.name ||
      ...
      job.workflow_name !== "CI" || job.head_sha !== SUCCESSOR_DIRECT_PARENT_SHA ||
...
function ineligibleV2RunExact(value) {
  return record(value) && value.id === INELIGIBLE_V2_RUN.runId &&
    ...
    value.head_sha === SUCCESSOR_DIRECT_PARENT_SHA &&
...
async function verifyIneligibleV2Candidate(fetchImpl, token) {
    ...
    [pullRequest, reviewedHead, run, jobs, archivedWorkflow] = await Promise.all([
      ...
      releaseGithubGet(
        ...
        `/contents/${CONTAINMENT_WORKFLOW_PATH}` +
          `?ref=${SUCCESSOR_DIRECT_PARENT_SHA}`,
      ),
    ]);
  } catch {
  ...
  if (!ineligibleV2PullRequestExact(pullRequest) ||
    ...
    reviewedHead.tree.sha !== SUCCESSOR_DIRECT_PARENT_TREE_SHA ||
...
function currentWriterNotStarted(jobs) {
  ...
  return writer.status === "queued" && writer.conclusion === null &&
...
async function verifyContainmentSingleUseAuthority(
  ...
  return Object.freeze({
    ...
    currentWorkflow: Object.freeze({
      ...
      totalDispatchRuns: runs.length,
      currentWriterNotStartedExact: true,
      solePriorRunNoWriteExact: recoveryBridge !== null &&
...
export async function verifyPostQReviewedCandidate(
  ...
  if (reviewedPullRequest.number <= INELIGIBLE_V2_RUN.pullRequestNumber) {
    fail("reviewed_candidate_invalid");
  }
  ...
  if (!Number.isFinite(currentStartedAt) || !Number.isFinite(mergedAt) ||
    !Number.isFinite(ineligibleRunCompletedAt) || !Number.isFinite(expiredDeadline) ||
    !Number.isFinite(nowMs) || currentStartedAt < expiredDeadline ||
    nowMs < expiredDeadline ||
    derivedDeadline === null || !Number.isFinite(deadline) ||
    mergedAt <= ineligibleRunCompletedAt || mergedAt > currentStartedAt ||
  ...
  let mainReference;
  try {
    [candidateCommit, predecessorCommit, preV2Commit, expiredRunHeadCommit,
  ...
  if (!record(candidateCommit) || candidateCommit.sha !== candidateSha ||
    ...
    !record(preV2Commit) ||
    preV2Commit.sha !== SUCCESSOR_DIRECT_PARENT_PARENT_SHA ||
    ...
    predecessorPullRequest.merge_commit_sha !== SUCCESSOR_DIRECT_PARENT_PARENT_SHA ||
  ...
  return Object.freeze({
    ...
    predecessorBridge: {
      ...
      preV2CandidateSha: SUCCESSOR_DIRECT_PARENT_PARENT_SHA,
      ...
      preV2MergeCommitSha: SUCCESSOR_DIRECT_PARENT_PARENT_SHA,
...
export async function verifyPermanentStagingPostQAuthorityV3(
  overrides = {},
) {
  const dependencies = {
  ...
  if (!record(reviewedCandidate) ||
    reviewedCandidate.schemaVersion !== POST_Q_REVIEWED_CANDIDATE_V3_SCHEMA) {
    fail("reviewed_candidate_invalid");
  }
  ...
  const authority = {
    schemaVersion: POST_Q_AUTHORITY_V3_SCHEMA,
    operation: "post-q-deployment-stop-containment-v3",
    repository: REPOSITORY,
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  verifyPermanentStagingPostQAuthorityV3().catch((error) => {
